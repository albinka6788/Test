...
@Updation History : 
   ...
   @Update-2 : {2016-Feb-19}, {Venkatesh}, { #1. Updating the quote information if quote number already exists
   @Update-3 : {2016-Feb-19}, {Venkatesh}, { #1. Setting isEmailVerified as false by default
   @Update-4 : {2016-Aug-10}, {Prem}, { Inserted new quote-id in "UserQuotes" table also to acheive one to many relationship for impersonation feature.
...
END
	...
	INSERT INTO [dbo].[Quote]
		...
		EXEC MaintainUserQuotesLinking @Quote_Identity,@SEQUENCEOrganisationUserDetailId,NULL,1,NULL,1,0 

	   END
	   ELSE
	   BEGIN
	   UPDATE [dbo].[Quote] SET [RequestDate] = @RequestDate,

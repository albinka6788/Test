...
namespace BHIC.Common.DataAccess
{
    public sealed class SqlHelper
    {
        ...
        /// <summary>
        /// This field will be required to fetch out the associated connection string 
        /// </summary>
        string databaseName = string.Empty;
        SqlConnection connection = null;
        SqlTransaction transaction = null;
        SqlHelperParameterCache objSqlHelperParameterCache = null;
        ...
        /// <returns></returns>
        private string GetConnectionString(string dbAppSettingName, string defaultValue = "", bool decryptValue = true)
        {
            if (string.IsNullOrWhiteSpace(dbAppSettingName))
                throw new ArgumentNullException("Database Name invalid");
            return (decryptValue ?
                (string.IsNullOrEmpty(ConfigurationManager.AppSettings[dbAppSettingName]) ? defaultValue :
                       Encryption.DecryptText(ConfigurationManager.AppSettings[dbAppSettingName], "")) :
                (string.IsNullOrEmpty(ConfigurationManager.AppSettings[dbAppSettingName]) ? defaultValue :
                        ConfigurationManager.AppSettings[dbAppSettingName]));
        }

        /// <summary>
        /// Get DBParamHelper Cache Instance
        /// </summary>
        /// <returns></returns>
        SqlHelperParameterCache GetDBHelperParamCache() {
            if (objSqlHelperParameterCache.IsNull())
                objSqlHelperParameterCache = new SqlHelperParameterCache();
            return objSqlHelperParameterCache;
        }
        ...
        public void OpenConnection()
        {
            ...
            {
                connection = new SqlConnection(GetConnectionString(databaseName));
            }
            if (connection.State != ConnectionState.Open)
            {
                connection.Open();
        ...
        public void BeginTransaction(bool throwError)
        {
            ...
            if (connection != null)
            {
                ...
                {
                    errorMessage = "Please open connection before performing any Transaction Activity";
                }
            }
            else
            {
        ...
        public void CommitTransaction(bool throwError)
        {
            ...
            if (connection != null)
            {
                if (connection.State != ConnectionState.Closed)
                {
                    ...
                    {
                        errorMessage = "Please Start a Transaction before submitting any changes";
                    }
                }
                else
                {
        ...
        public void RollbackTransaction(bool throwError)
        {
            ...
            if (connection != null)
            {
                if (connection.State != ConnectionState.Closed)
                {
                    ...
                    {
                        errorMessage = "Please Start a Transaction before reverting any changes";
                    }
                }
                else
                {
        ...
        public int ExecuteNonQuery(string spName, params object[] parameterValues)
        {
            ...
            if ((parameterValues != null) && (parameterValues.Length > 0))
            {
                ...
                SqlParameter[] commandParameters = GetDBHelperParamCache().GetSpParameterSet(GetConnectionString(databaseName), spName);
        ...
        public int ExecuteNonQuery(SqlConnection connection, string spName, params object[] parameterValues)
        {
            ...
            {
                // Pull the parameters for this stored procedure from the parameter cache (or discover them & populate the cache)
                SqlParameter[] commandParameters = GetDBHelperParamCache().GetSpParameterSet(connection, spName);
        ...
        public int ExecuteNonQuery(SqlTransaction transaction, string spName, params object[] parameterValues)
        {
            ...
            {
                // Pull the parameters for this stored procedure from the parameter cache (or discover them & populate the cache)
                SqlParameter[] commandParameters = GetDBHelperParamCache().GetSpParameterSet(transaction.Connection, spName);
        ...
        public int ExecuteNonQueryTypedParams(String connectionString, String spName, DataRow dataRow)
        {
            ...
            {
                // Pull the parameters for this stored procedure from the parameter cache (or discover them & populate the cache)
                SqlParameter[] commandParameters = GetDBHelperParamCache().GetSpParameterSet(connectionString, spName);
